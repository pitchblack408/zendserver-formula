...
addMainZendClusterNode:
  cmd.run:
    - name: /etc/zendserver/addServerToCluster.py {{ zend_main_node_name }} {{ zend_main_node_ip }} {{ zend_blueprint_dbhost }} {{ zend_blueprint_dbusername }} {{ zend_blueprint_dbpassword }} {{ zend_blueprint_dbschema }} admin {{ zend_api_key }}
    - cwd: /
    - stateful: True
    - require:
